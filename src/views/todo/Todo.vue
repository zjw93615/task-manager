            ...
            <component :is="scrollbarTag" class="todo-content-scroll-area" :settings="settings" ref="taskListPS" :key="$vs.rtl">
                ...
                <transition-group class="todo-list" name="list-enter-up" tag="ul" appear>
                    <li
                      ...
                      :data-key="String(currFilter) + String(task._id)"
                      :key="String(currFilter) + String(task._id)"
                      :style="[{transitionDelay: (index * 0.1) + 's'}]">

                      <todo-task :taskId="task._id" @showDisplayPrompt="showDisplayPrompt($event)" :key="String(task.title) + String(task.desc)" />
                        <!--
